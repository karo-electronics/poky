...
# [ df mktemp base32 base64 gets a special treatment and is not included in this
bindir_progs = "arch b2sum basename chcon cksum comm csplit cut dir dircolors dirname du \
                env expand expr factor fmt fold groups head hostid id install \
                join link logname md5sum mkfifo nice nl nohup nproc numfmt od paste pathchk \
...
do_install_append() {
	for i in df mktemp base32 base64; do mv ${D}${bindir}/$i ${D}${bindir}/$i.${BPN}; done
...
ALTERNATIVE_${PN} = "lbracket ${bindir_progs} ${base_bindir_progs} ${sbindir_progs} base32 base64 mktemp df"
ALTERNATIVE_${PN}-doc = "base32.1 base64.1 mktemp.1 df.1 lbracket.1 groups.1 kill.1 uptime.1 stat.1  hostname.1"
